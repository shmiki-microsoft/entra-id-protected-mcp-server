...
- 🔐 **Microsoft Entra ID 認証**: JWT トークンの署名検証、audience/issuer 検証、スコープまたはロール検証
...
- [デバッグとテスト](#デバッグとテスト)
- [開発ガイド](#開発ガイド)
- [Azure App Service へのデプロイ](#azure-app-service-へのデプロイ)
...
- Microsoft Entra ID の JWT トークンの包括的な検証
  ...
  - 必須スコープ (`ENTRA_REQUIRED_SCOPES`) または必須ロール (`ENTRA_REQUIRED_ROLES`) のいずれかを満たすか検証
  - 検証成功時は、どの scope / role で通過したかを INFO ログに出力
...
| `auth/entra_auth_provider.py` | JWT トークンの検証。JWKS を取得して署名検証、audience/issuer/スコープまたはロールのチェックを実施 |
...
### 1. Microsoft Entra ID のアプリ登録とスコープ / アプリロール作成
...
#### 1-5. アプリロール (`access_as_application`) の作成

1. 対象アプリの **「アプリ ロール」** を開く
2. **「アプリ ロールの作成」** をクリック
3. 以下を入力:
   - **表示名**: `access_as_application`
   - **許可されるメンバーの種類**: **アプリケーション**
   - **値**: `access_as_application`
   - **説明**: 適切な説明を入力
4. **有効化** をオンにして保存
5. ロール値 `access_as_application` をメモしておきます（後で `ENTRA_REQUIRED_ROLES` として `.env` に設定します）

> **💡 補足**: このロールは、クライアント資格情報フローなどで取得したトークンの `roles` クレームに含まれます。

#### 1-6. クライアント シークレットの作成 (OBO フロー用)
...
ロールベースアクセス制御 (RBAC) を実装する場合は、追加のアプリロールを定義してユーザーに割り当てます。なお、MCP サーバーの認証用ロール `access_as_application` は前章の手順で作成済みである想定です。RBAC 機能が不要な場合は、このステップをスキップできます。
...
実際のリクエスト処理時には、以下のような成功ログも出力されます:

```
INFO auth.entra_auth_provider: Token validation succeeded via scopes: access_as_user
```
...
3. **トークン検証**: `EntraIDAuthProvider` が以下を検証
   ...
   - 必須スコープ (`ENTRA_REQUIRED_SCOPES`) または必須ロール (`ENTRA_REQUIRED_ROLES`) のいずれかを満たしているか
   - 検証成功時は、マッチした scope / role を INFO ログに記録
...
7. **エラーハンドリング**: トークンが無効・期限切れ・必須スコープ / ロール不足の場合は `AuthenticationError` が発生
...
`ENTRA_REQUIRED_SCOPES` と `ENTRA_REQUIRED_ROLES` はカンマ区切りで列挙できます:

```dotenv
ENTRA_REQUIRED_SCOPES=access_as_user, User.Read, files.read
ENTRA_REQUIRED_ROLES=access_as_application, Admin
```

両方を設定した場合、トークン検証は **必要スコープを満たす** または **必要ロールを満たす** のどちらかで成功します。
...
結果:
- `ENTRA_REQUIRED_SCOPES` → `["access_as_user", "files.read", "user.read"]`
- `ENTRA_REQUIRED_ROLES` → `["access_as_application", "admin"]`
...
| `missing_required_permissions` | 必須スコープ / ロール不足 | トークン検証時 |
| `insufficient_role` | ロール不足 | RBAC ツール呼び出し時 |
| `obo_token_acquisition_failed` | OBO フロー失敗 | Azure/Graph API アクセス時 |

## Azure App Service へのデプロイ

このサーバーを Azure App Service にデプロイして、クラウド環境で実行し、リモート MCP サーバーとして使えるようにもできます。

### 1. Azure Portal で App Service を作成

#### 1-1. App Service の新規作成

1. Azure Portal にログインし、**「App Services」** を検索して選択
2. **「+ 作成」** → **「Web App」** をクリック
3. 以下の項目を設定:
   - **サブスクリプション**: 使用するサブスクリプションを選択
   - **リソースグループ**: 既存のものを選択、または新規作成
   - **名前**: アプリの一意な名前を入力 (例: `entra-mcp-server`)
   - **公開**: **「コード」** を選択
   - **ランタイムスタック**: **「Python 3.12」** を選択
   - **オペレーティングシステム**: **「Linux」** を選択
   - **地域**: 任意の地域を選択 (例: Japan East)
4. **App Service プラン** を選択または新規作成 (例: Basic B1 など)
5. **「確認および作成」** → **「作成」** をクリック

#### 1-2. 環境変数の設定

App Service が作成されたら、環境変数を設定します。

1. 作成した App Service のページに移動
2. 左側メニューから **「設定」> [環境変数]** を選択
3. **「アプリケーション設定」** タブで **「+追加」** をクリック
4. 以下の環境変数を追加:

| 名前 | 値 | 説明 |
|------|-----|------|
| `ENTRA_TENANT_ID` | `your-tenant-id` | Microsoft Entra ID テナント ID |
| `ENTRA_APP_CLIENT_ID` | `api://your-client-id` | アプリケーション (クライアント) ID または API の URI |
| `ENTRA_REQUIRED_SCOPES` | `access_as_user` | 必須スコープ (カンマ区切り) |
| `ENTRA_REQUIRED_ROLES` | `access_as_application` | 必須アプリロール (カンマ区切り) |
| `ENTRA_APP_CLIENT_SECRET` | `your-client-secret` | クライアントシークレット (OBO フロー用) |
| `APP_LOG_LEVEL` | `INFO` | アプリケーションログレベル |
| `ENTRA_AUTH_LOG_LEVEL` | `INFO` | 認証ログレベル |
| `MCP_SERVER_LOG_LEVEL` | `INFO` | MCP サーバーログレベル |
| `MCP_TRANSPORT` | `streamable-http` | MCP トランスポート方式 |
| `MCP_HOST` | `0.0.0.0` | **重要**: App Service では `0.0.0.0` に設定 |
| `MCP_PORT` | `8000` | MCP サーバーポート |

> **⚠️ 重要**: `MCP_HOST` は App Service では必ず **`0.0.0.0`** に設定してください。`localhost` では外部からアクセスできません。

5. **「保存」** をクリックして設定を保存

#### 1-3. スタートアップコマンドの設定

App Service に Python アプリを起動するコマンドを指定します。

1. **「構成」** ページの **「全般設定」** タブを選択
2. **「スタートアップコマンド」** フィールドに以下を入力:

```bash
python main.py
```

> **注**: 後述の ZIP デプロイでは `src/` フォルダの内容が `/home/site/wwwroot` にデプロイされます。そのため、`main.py` は `/home/site/wwwroot/main.py` として配置され、上記のコマンドで正常に起動します。

3. **「保存」** をクリック

### 2. VS Code の Azure Tools 拡張機能を使った ZIP デプロイ

#### 2-1. VS Code に Azure App Service 拡張機能をインストール

1. VS Code を開く
2. 拡張機能ビューを開く (Ctrl+Shift+X / Cmd+Shift+X)
3. **「Azure App Service」** を検索してインストール
4. **「Azure Resources」** 拡張機能も自動的にインストールされます

#### 2-2. Azure にサインイン

1. VS Code の左側のアクティビティバーで **Azure アイコン** をクリック
2. **「Sign in to Azure...」** をクリック
3. ブラウザが開くので、Azure アカウントでサインイン

#### 2-3. デプロイ用の準備

デプロイ前に、`requirements.txt` を作成して `src/` 配下に配置します。

> **💡 ヒント**: `src/` フォルダーだけを App Service にアップロードするため、依存関係ファイルも `src/` 内に配置する必要があります。
...
2. 以下を確認:
   - `src/` ディレクトリにアプリケーションコードが配置されている
   - `src/requirements.txt` が存在する
   - `.env` ファイルは **含めない**（環境変数は App Service の構成で設定済み）

#### 2-4. ZIP デプロイの実行

1. VS Code でプロジェクトを開く
2. 左側のアクティビティバーで **Azure アイコン** をクリック
3. **「RESOURCES」** セクションを展開
4. サブスクリプションを展開 → **「App Services」** を展開
5. 作成した App Service (例: `entra-mcp-server`) を右クリック
6. **「Deploy to Web App...」** を選択
7. デプロイするフォルダを選択 → **`src` フォルダを選択**
8. 確認ダイアログで **「Deploy」** をクリック

デプロイが完了すると、VS Code の出力パネルにログが表示されます:
...
1. Azure Portal で App Service のページに移動
2. **「概要」** ページの **URL** をクリックしてアプリにアクセス
3. ログを確認するには、**「ログストリーム」** を開く
